...
if ($method == 'history_checker_table') {
    ...
    if (!empty($date_from) && !empty($date_to)) {
        $conditions[] = "DATE(a.checked_date) BETWEEN :date_from AND :date_to";
    }
    if (!empty($search_by)) {
    ...
    if ($stmt->rowCount() > 0) {
        ...
        $data .= '<tr>';
        $data .= '<td colspan="4" class="text-center">Nothing found.</td>';
        $data .= '</tr>';
    }

    // Check if there are more rows beyond the current page
    $nextOffset = $offset + $rowsPerPage;
    $sql_more = "SELECT 1 FROM t_training_record a 
    ...
    $stmt_more->bindParam(':checker_name', $checker_name, PDO::PARAM_STR);
