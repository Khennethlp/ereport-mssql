...
if ($method == 'load_viewer_data') {
    ...
    if (!empty($search_by_serial)) {
        $stmt_more->bindParam(':search_by_serial', $search_by_serial, PDO::PARAM_STR);
    }
    if (!empty($search_by_batch)) {
        $stmt_more->bindParam(':search_by_batch', $search_by_batch, PDO::PARAM_STR);
    }
    if (!empty($search_by_group)) {
        $stmt_more->bindParam(':search_by_group', $search_by_group, PDO::PARAM_STR);
    }
    if (!empty($search_by_training)) {
        $stmt_more->bindParam(':search_by_training', $search_by_training, PDO::PARAM_STR);
    }
    if (!empty($search_by_filename)) {
        $stmt_more->bindParam(':search_by_filename', $search_by_filename, PDO::PARAM_STR);
    }
    if (!empty($date_from) && !empty($date_to)) {
